...
export default function Escalafon() {
  ...
  const toggleGrupo = (grupo) => {
    setAbierto((prev) => (prev === grupo ? null : grupo));
  };
      ...
      <FullPageLayout>
        ...
        <div className="max-w-6xl mx-auto px-6 pt-28">
          ...
          {Object.entries(grupos).map(([grupo, filas]) => (
            <div key={grupo} className="mb-6 border border-yellow-500 rounded-xl shadow overflow-hidden">
              <button
                onClick={() => toggleGrupo(grupo)}
                className={`w-full text-left px-4 py-3 font-bold text-lg flex justify-between items-center ${grupoColores[grupo]}`}
              >
                {grupo}
